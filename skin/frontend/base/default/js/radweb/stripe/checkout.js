...
IWD.OPC = {
		...
		savePayment: function(){
			...
			if (payment.currentMethod != 'stripe' 
				&& payment.currentMethod != 'radweb_stripe' 
				&& payment.currentMethod != 'radweb_stripe_stripe') {
				var form = $j('#co-payment-form').serializeArray();
				
				IWD.OPC.Checkout.xhr = $j.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/savePayment',form, IWD.OPC.preparePaymentResponse,'json');
			}else if(payment.currentMethod == 'stripe') {
			...
			} else if(payment.currentMethod == 'radweb_stripe') {
				var stripe_card = $('stripe_card').value;
            	if(stripe_card == -1) {
					Stripe.createToken({
					...
					jQuery.ajax({
	                    url: IWD.OPC.Checkout.config.baseUrl + 'manage-cards/stripe/prepareSavePayment',
	                    type: 'post',
	                    dataType: 'json',
	                    data: { stripe_card: stripe_card },
	                    success : function(data) {
	                    	$('stripeToken').value = data.token;
	                    }
	                });
				}
				var form = $j('#co-payment-form').serializeArray();
				IWD.OPC.Checkout.xhr = $j.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/savePayment',form, IWD.OPC.preparePaymentResponse,'json');
			} else if(payment.currentMethod == 'radweb_stripe_stripe') {
				var stripe_card = $('stripe_card').value;
            	if(stripe_card == -1) {
            		Stripe.createToken({
						name: $('radweb_stripe_stripe_cc_owner').value,
						number: $('radweb_stripe_stripe_cc_number').value,
						cvc: $('radweb_stripe_stripe_cc_cid').value,
						exp_month: $('radweb_stripe_stripe_expiration').value,
						exp_year: $('radweb_stripe_stripe_expiration_yr').value
					}, function(status, response) {
						if (response.error) {
							IWD.OPC.Checkout.hideLoader();
							IWD.OPC.Checkout.xhr = null;
							...
							$('stripeToken').value = response['id'];
							var customerPassword = $('billing:customer_password').value;
				            var confirmPassword = $('billing:confirm_password').value;
				            if((typeof customerPassword !== 'undefined') 
				                && (customerPassword != '')
				                && (typeof confirmPassword !== 'undefined') 
				                && (confirmPassword != '')
				                && (customerPassword == confirmPassword)) {

				                jQuery.ajax({
				                    url: IWD.OPC.Checkout.config.baseUrl + 'manage-cards/stripe/saveCustomerPassword',
				                    type: 'post',
				                    dataType: 'json',
