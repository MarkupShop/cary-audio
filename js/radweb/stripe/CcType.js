...
$j(document).ready(function() {
    $j(document).on('change', '#radweb_stripe_stripe_cc_number', function() {
    // $j('#radweb_stripe_stripe_cc_number').change(function() {
        $j('#stripe_cards li').each(function() {
            if($j(this).hasClass('on')) {
                $j('#type-of-card').val($j(this).attr('class'));
            }
        });
    });
});
